...
### 2.1 跨房间条件型（2 条，已有表但缺主力路径注入）

| 干员 | buff_id | 效果 | 现状 |
|------|---------|------|------|
| 深巡 | `trade_ord_spd_ext[000]` (E0) | 基础 +25%，乌尔比安在基建 +5% | `_B_CROSS_ROOM_PAIR_TABLE` 中已有 |
| 深巡 | `trade_ord_spd_ext[001]` (E2) | 基础 +30%，乌尔比安在基建 +5% | 同上 |
| 贝洛内 | `trade_ord_spd_ext[020]` (E0) | 基础 +25%，伺夜在基建 +5% | 同上 |
| 贝洛内 | `trade_ord_spd_ext[021]` (E2) | 基础 +30%，伺夜在基建 +5% | 同上 |

> 基础效率部分已通过 `best_efficiency()` 覆盖。跨房间 +5% 已通过 `synergy_cross_room_pair` 定义，但主力求解路径（Mfg/Trade 穷举 + 贪心）未传 `all_assignments`，导致 B7 层评估缺失。当前仅 `refine.py` post-processing 使用。
...
贸易站     117    113      1       3     3 条跨房间(已有表缺注入)+1 条心情P1
