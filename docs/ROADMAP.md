...
## Step 1: 满练全图鉴 — 基线验证 ✅ 已实现

### 实现状态
- 求解器: `steward_core/solver.py` — 单班次贪心，OpFilter 协议可扩展
- 产出计算: `steward_core/production.py` — 基于 PRTS 公式的精确日产出（含无人机加速）
- 入口: `run_solver.py --maa-path <路径>`
...
- 产物配置: 2经验房 + 2赤金房 + 2贸易站，经验优先于赤金（经验完全依赖基建，赤金超出贸易站消耗价值归零）

### 当前结果
- 作战记录: 48.1 个/天（含无人机加速 Mfg[0] +80%）
- 赤金制造: 78.3 个/天
- 龙门币: 39,160 /天
- 对比基准模板匹配率: 10.3%（纯贪心 vs 手工联动调优，符合预期）
