...
> **实现注**：槽位 ID 通过 `context.py:_make_slot_id()` 生成为 `{prefix}_{room}_{slot}` 格式（如 `mfg_0_0`），产品类型存储在 `SlotAssignment.product` 而非 ID 中。
...
效果组定义了技能效果的传播范围。同设施相邻槽位构成效果组（如一间制造站的 3 个槽位），全局注入作用于全设施同类槽位，同宿舍槽位共享恢复 buff。槽位间通过同组存在性、阵营计数、技能类别计数、全局阵营计数、跨组目标等方式引用。
```
...
以上 `xp_lmd_ratio`、`reception_to_mfg_ratio`、`office_to_mfg_ratio`、`drone_to_mfg_ratio`（§1.4）均为**工程常量**——源自玩家社区长期实践共识，参数化仅为代码可维护性，并非需要校准的可变参数。这些比值已在人类排班决策中隐式使用多年，本项目目标为逼近人类排班水平，直接采用逻辑自洽。

> **§2 槽位体系**（槽位标识、效果组、槽位间引用）已合并至 §1.1。
...
| 类型 | 作用 | 输出 | 求值位置 |
|------|------|------|---------|
| 1 (a-m) | 槽位效率 | `list[LinearSegment]` | `evaluate_room()` |
| 2 | 全局状态增量 | 修改 S 的 5 个维度 | `compute_buff_pool()` |
| 3 | 规则修正 | 全局注入/per-op修正 | `compute_control_global_bonus()` |
| 4 | 槽位屏蔽 | 归零同组槽位 | `evaluate_room()` 优先 |
| 5a | 心情消耗修正 | 修改 `mood_burn` | `compute_mood_modifiers()` |
| 5b | 宿舍恢复 | `recovery_rate` | `evaluate_dorm_recovery()` |
| 6 | 订单参数 | 三类机制（独占/概率/分布） | `_get_trade_order_multiplier()` |
| 7 | 纯时间加速 | 无人机缩放 `effective_hours` | `_calc_drone_daily()` |

全局状态向量（类型 1f 读取，类型 2 写入）：
...
> 具体的 buff_id 映射和分类逻辑由代码注册表驱动（`synergy/types.py` TABLES 注册器 + §7 分类规则）。
...
统一计价单位：**LMD 等值/天**。赤金 = 500 LMD/个，经验通过 `xp_lmd_ratio` 折算（243 布局 1.3:1）。

**求值顺序**（类型 4 优先，在效率叠加前确定屏蔽集）：
...
**各设施对总产能的贡献**见 §8.4 phase_mfg/phase_trade/phase_control/phase_remaining 的 contribution 公式。
...
类型 2 的令/夕（mood 阈值切换写入状态维度）、类型 1g 的铅踝（mood 落差影响效率）在窗口内
经历连续心情变化。当前代码使用布尔门控近似（`mood >= 12` → 写入 A 维度, `< 12` → 写入 B 维度；
铅踝通过 `mood_ctx.qianhuai_mood(names)` 传递初始心情给 `stepped_efficiency()`），
而非连续展平公式。连续展平方案见 `mood_flow.py` 中 mood 相关注释。
...
### 7.1 分类规则（按 buff_id 前缀 + room_type）
...
> **注**：以上完备性自检基于人工审查。分类规则编码后（§7.1）将通过自动化完备性测试验证。
...
```
初始化: A₀ = 空分配（第一轮迭代从空池直接穷举），S₀[w] = 0
        或 S₀_max 冷启动（离散启动点，见 §8.6）
        D₀[w][d] = δP/δS[w]|_{S=S₀, A=A₀}

循环 k (状态不动点迭代):
  对每个窗口 w ∈ [1..W]:
    ― phase_mfg: Mfg 效果组穷举（含完整联动求值） ―
    对每间制造站效果组（CR×2 + PG×2）:
    ...
    ― phase_trade: Trade 效果组穷举（含类型 6 精确计算） ―
    对每间贸易站效果组（×2）:
    ...
    > **设计决策：类型 3 全局注入的后置求值**。phase_control 在 phase_mfg/phase_trade 之后执行，
    > "受影响槽位数"取已填充的 Mfg/Trade 槽位精确值，无循环依赖。后置的另一层意图是让类型 3
    > 注入者（阿米娅 +42% Trade、杜宾 +30% 三星 Mfg 等）获得其完整边际价值评估——它们的贡献
    > 乘以精确槽位数后可能远超单个 Control 槽位的替代者。下一轮迭代中，phase_mfg/phase_trade 将
    ...
    ― phase_remaining: Power/Office/Reception/Dormitory 槽位级贪心 ―
    ...
    **Dormitory**:
      contribution(op, w) = Σ(写入S[w][d] × D_k[w][d])              ← 类型 2
                          + 自身恢复值 + 室友边际增量                 ← mood-driven 恢复
                          × eff_weight  (eff_weight = max(peak_eff/30, 0.1))
...
对每个窗口 w、每个全局状态维度 d:
  D[w][d] = 0
  遍历窗口 w 中 phase_mfg/phase_trade 已分配的 Mfg/Trade 槽位:
...
```
启动点 1: S₀_max（冷启动，全部状态维度乐观上界）
启动点 2-4: 随机可行分配（增加覆盖度）

对每个启动点运行状态迭代 → 取 max P(A*)
```

冷启动的开销 ≈ 5 轮迭代 ≈ 5 × W × (Mfg 1140×4 + Trade 220×2 + Control 15×5 + 其余 30) ≈ 5W × 6000 次轻量评估（W=窗口数）。每窗口每轮穷举已在 phase_mfg/phase_trade 完成，总计算量可控。
...
记忆机制（§8.4, §8.8）保证算法有限步终止于关于 Phase 分解邻域 N(A) 的局部最优——即"任何单 Phase 改变都无法提升 P"。联合扰动（§8.4 记忆检查末尾）将邻域扩展至已知的跨 Phase 耦合对（类型 1f 读取者 ↔ 类型 2 写入者，~25 对），进一步缩小潜在鞍点范围。坐标下降在非凸分配空间上不能保证全局最优，多启动策略增加覆盖度但非穷举。
...
N(A) 定义（§8.4）将邻域限定为单 Phase 变化 + 联合扰动覆盖的跨 Phase 耦合对（~25 对）。若存在未覆盖的鞍点——如必须同时改两个不在耦合对清单中的 Phase 才能提升 P——则 N(A) 无法发现该提升。耦合对清单覆盖了所有已知的类型 1f 读取者—类型 2 写入者关系，未覆盖的鞍点概率极低。
...
§8.6 指出 D[d] 是分段常数——仅当读取者入场/退场时才变化。截云（`wushu_crystal = yanhuo // 5`）在 S[yanhuo] 跨越 5 的倍数时 D 突变。至简的 `engineering_robots // 8` 在满级布局中恒为 64（触及代码上限 64），消费量恒为 40%，不构成例外。工程上可接受（仅影响类型 2 写入者的初始估值，迭代会修正）。

#### 冷启动：可选加速手段

S₀_max 冷启动（§8.6）的 D₀ 基于"所有类型 2 写入者均在场"的乐观假设，可能高估某些状态维度的边际价值，导致首轮分配偏向类型 1f 读取者。此偏向在后续迭代中可能被修正，也可能因槽位级贪心逐轮不可回退而固化。

默认使用空分配启动（第一轮迭代从空池直接穷举），冷启动作为可选加速手段。多启动策略（§8.6）进一步增加覆盖度。建议默认使用空分配，冷启动作为可选的"追求最优性"开关。
...
| Reception 直接效率 | 通过信用商店链折算为等效 Mfg 效率（§1.4），参数化 |
...
定义**记忆增强转移函数** T_mem。在原始转移 T（§8.4 循环体）的基础上，T_mem 维护一个已访问状态集合 V：
...
其中 N(A) 为 §8.4 定义的 Phase 分解邻域，|N(A)| ≤ 1800。

**V 集合的等价性定义**：A₁ ≡ A₂ 当且仅当它们在每个槽位的干员分配完全相同——即标准化为槽位 ID → 干员 ID 的映射后逐键比较。迭代开始前 V 初始为空集，首轮迭代后 A_k 加入 V。SlotSolver 直接操作槽位映射，无需展平转换。
...
H1-H2 由游戏系统保证（有限干员池/槽位），H3-H4 为建模假设（已在 §8.7 登记例外）。

---
### 8.9 验证结果

- **收敛**：14×12h 真实数据集上空分配启动 1-2 轮、冷启动 ≤5 轮收敛
- **解质量**：SlotSolver 典型场景较空分配贪心基线提升 +1,356 LMD 等值
  （详见 `archive/slot-iteration-record.md` 第四期验收）
