...
**对 A 层注入**: BuffPool 作为 GlobalContext 的一部分传入 `synergy_efficiency()`，各体系按需查询。

**MVP 特殊处理**: 宿舍干员列表需由求解器确定。但中枢与宿舍的 29 个工位属于同一排班方案 → 存在循环依赖（需要知道宿舍有哪些人才能算 buff 池，但宿舍人选又依赖效率排序）。求解策略：

1. **先固定中枢**: 使用社区公认最优方案（令+重岳+夕+凯尔希+焰尾）
2. **后算宿舍**: 按恢复速率需求选人填充宿舍 20 工位
3. **缓存 BuffPool**: 贪心填充阶段 BuffPool 不变
...
def compute_global_burn(
    ...
    MVP 单班次 12h → burn 为常数，不调用此函数计算 t_red
    """
```
