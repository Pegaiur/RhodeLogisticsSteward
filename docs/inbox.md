...
- [ ] **订单机制互斥的算法级建模** — 可露希尔(trade_ord_closure)特别订单覆盖低语(trade_ord_vodfox)，当前未建模，导致求解器将巫恋+可露希尔选入同一房间（游戏内低语被覆盖不生效，但求解器仍计入 whisper 90% 效率加成）。更广范围：可露希尔/但书(trade_ord_law)/龙舌兰(trade_ord_long)/裁缝(trade_ord_wt)之间也存在优先级链（closure > law > long > tailor），`_get_trade_order_multiplier` 已建模订单侧优先级但归零侧(whisper)遗漏。推荐路径：引入集中式 `_ROOM_MECHANISM_CONFLICTS` 声明表 + `resolve_mechanism_conflicts()` 统一解析函数，纳入 synergy/types.py TABLES 注册器，使 evaluate_room + opportunity.py 共用同一 truth source。不应在各 synergy 函数中散落 guard——见 `synergy/mfg_linkages.py` synergy_whisper、`solver/slot/opportunity.py` _detect_mode — 2026-05-31 — `synergy/types.py` + `synergy/mfg_linkages.py` + `evaluate.py` + `slot/opportunity.py`
