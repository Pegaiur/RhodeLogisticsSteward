...
- [ ] **interval_hours 外生日历约束去除** — `SolverParams.interval_hours=8.0` 是固定班间间隔，压制了 `dorm_recovery()` 已建模的差异化恢复信息：快恢复干员溢出浪费、慢恢复干员吃不饱、宿管价值被 `min(24.0, ...)` 上限截断。λ 影子乘子无法区分"恢复快可多用"vs"恢复慢需多休"。应改为内生决策——求解器自行决定窗口间恢复时长，或放宽为"恢复到全员 ready"的非等间隔模式。涉及 `slot/solver.py` 的 `after_recovery` 调用和 `period_from`/`period_to` 输出时段计算 — 2026-05-31 — `mood_flow.py` + `slot/solver.py` + `params.py`
