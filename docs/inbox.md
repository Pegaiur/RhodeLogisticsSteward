...
- [x] **interval_hours 外生日历约束去除** — ~~`SolverParams.interval_hours=8.0` 是固定班间间隔，压制了 `dorm_recovery()` 已建模的差异化恢复信息~~ **（已修复, 2026-05-31）**。根因诊断发现 `interval_hours=8.0` 是建模错误——游戏内班次之间无间隔（结束即开始），恢复通过宿舍在位实现。修改：默认值改为 0，移除 report.py / run_solver.py / test_solver_e2e.py 硬编码，solve_slot 中 `after_recovery` 加 `interval_hours>0` 守卫。见 `5dc66ac` 和 `params.py` 默认值修正 — `mood_flow.py` + `slot/solver.py` + `params.py`
