...
def evaluate_dorm_recovery(
    ...
    *,
    ...
    Args:
        ...
        dorm_level: 宿舍等级（1-5），用于基础恢复
        dorm_ambiance_per_room: 每间宿舍氛围值

    Returns:
        每小时心情恢复量

    聚合规则（按优先级）：
    0. 宿舍基础恢复: 1.5 + 0.1 × dorm_level + 0.0004 × ambiance
    1. 菲亚梅塔自律 (dorm_recExcludeOther): +2.0/h，隔离外部加成（含基础恢复）
    ...
    """
    # Rule 1: 菲亚梅塔自律 — 固定 +2.0/h，隔离外部加成（含基础恢复）
    ...
    # Rule 0: 宿舍基础恢复 — 1.5 + 0.1 × 等级 + 0.0004 × 氛围
    if dorm_ops:
        total += 1.5 + 0.1 * dorm_level + 0.0004 * dorm_ambiance_per_room
